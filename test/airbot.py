...
class AirbotPlayBase(PipelineEnv):
    def __init__(
        ...
        healthy_reward = 1.0,
        max_vel = 0.4,
        endpoint_min_z_pos = 0.8,
        noise_scale = 1e-2,
        reset_action_scale = 0.03,
    ...
    def reset(self, rng: jp.ndarray) -> State:
        ...
        # joint_ctrl = joint_ctrl.at[1].set(-0.3)
        # data = self.pipeline_init(qpos, qvel, ctrl = joint_ctrl)
        ...
        # data = data.replace(
        #     xpos=data.xpos.at[self.target_pos_id : self.target_pos_id + 3].set(target_pos))
        
        # data = data.replace(
        #     xpos=data.xpos.at[self.cube_id : self.cube_id + 3].set(cube_pos))

        # target_pos = jp.array([0.5364427, 0.10352592, 0.81999997])
    ...
    def step(self, state: State, action: jp.ndarray) -> State:
        ...
        angle_to_box = jp.arctan2(delta_y, delta_x + 0.00001)  # 计算角度差

        box_target_dis_0 = jp.linalg.norm(state.info['target_pos'] - data0.xpos[self.cube_id])
        temp = jp.where(box_target_dis_0 < 0.03, state.info['last_action'], -angle_to_box + action[0] + 1.5708)
        action =  action.at[4].set(temp)

        state.info['last_action'] = action[4]
        ...
        box_target_dis = jp.where(box_target_dis < 0.003, 0.0, box_target_dis)
        push_reward = 1 / (1 + 3 * box_target_dis)
        push_reward = push_reward * self.push_reward_weight

        # 任务完成奖励
        task_complete_reward = jp.where(box_target_dis < 0.003, 5.0, 0)  # 只有在接近目标时才给予奖励

        # ctrl_cost = self.ctrl_cost_weight * jp.sum(jp.square(data1.ctrl))
        ctrl_cost = 0.0
        ...
        cube_pos_xy = state.info['new_cube_pos']#cube_pos[0:2]
        cube_pos_x = cube_pos[0]
        cube_pos_y = cube_pos[1]
        ...
        health_reward = self.healthy_reward * jp.abs(done - 1.0)

        reward = push_reward + siet2cube_reward + health_reward + task_complete_reward + site_z_reward# - collision_cost# - cost_degree_cost# - box_still_cost - vel_box_cost  health_reward + 
        done = 0.0
        done = jp.where(box_target_dis < 0.003, 1.0, done)
