...
def compute_ppo_loss(
  ...
  task_loss = policy_loss + v_loss + entropy_loss

  ### calculate sim2real loss

  # A2 is the dataset simulated with post-tuned env at iteration k
  # A1 is the data generated by new policy plus dataset simulated with post-tuned env at iteration k
  
  n = data.observation.shape[2]
  m = data.action.shape[2]
  KLD, p_A2, A2 = past_data
  reshaped_obs = jnp.reshape(data.observation,(-1,n))
  reshaped_actions = jnp.reshape(data.action,(-1,m))
  reshaped_next_obs =jnp.reshape(data.observation,(-1,n))
  current_data = jnp.hstack([reshaped_obs, reshaped_actions, reshaped_next_obs])

  A1 = jnp.vstack([A2, current_data])
  print(A1.shape)

  dim = n + m + n
  num_samples = 10  # Number of random samples
  key = jax.random.PRNGKey(0)
  grid = jax.random.uniform(key, (num_samples, dim), minval=-3, maxval=3)

  p_A1 = dp.evaluate_kde(A1, grid=grid, bandwidth=0.1 )
  WD = dp.wasserstein_distance(p_A1, p_A2)
  sim2real_loss = -1 * KLD * WD
  
  total_loss = task_loss +sim2real_loss
