...
def compute_ppo_loss(
    ...
    rsr_loss_scale: float = 1.0) -> Tuple[jnp.ndarray, types.Metrics]:
  """Computes PPO loss.
  ...
  # Use an action produced by the policy currently being optimized.  The
  # rollout action in ``data`` was generated by an older policy and is constant
  # with respect to ``params``, so using it would make this term have zero
  # policy gradient.
  current_policy_actions = parametric_action_distribution.mode(policy_logits)
  sim2real_loss, distribution_distance = rsr.compute_rsr_loss(
      data.observation,
  ...
  return total_loss, {
      ...
      'sim2real_loss': sim2real_loss,
      'rsr_distribution_distance': distribution_distance,
  }
